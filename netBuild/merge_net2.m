function [layers, weights] = merge_net2(LAYERS, WEIGHTS, ratio )
%MERGE_NET merge multiple PWL networks
%   syntax:
%           [layers, weights] = merge_net2(LAYERS, WEIGHTS, ratio )
%   input:
%           LAYERS: n*1 struct, fields are B, stemB, id_layer, n is the
%                   number of sub nets
...
%                   column is the stemB , n is the number of sub nets, m is the number of nodes in each sub nets)
...
if isstruct(LAYERS)
    ...
    tmp = cell(num_nets, 3);
    for i = 1:num_nets
        tmp{i} = [LAYERS(i).B num2cell(LAYERS(i).id_layer, 2) num2cell(LAYERS(i).stemB, 2)];
    end
    LAYERS = tmp;
    mode = 'struct';
end
...
%% Data in each sub-network
num_nodes_separate = zeros(num_nets, 1); % number of nodes in each sub-network
for i = 1:num_nets
...
%% merge all the networks (raw edition)
...
% others
weights = [weight_constant; adjacency(:, end)];
if length(weights) ~= (size(layers, 1) + 1)
    disp('error')
end
stem = adjacency_to_stem(adjacency(:, 1:end-1));
if strcmp(mode, 'cell')
    ...
    layers = struct( 'id_layer', id_layer, 'stemB', stem, 'nx', nx, 'nNode', size(stem, 1), 'nLayer', max(id_layer));
    layers.B = B;
end